...
export default function Play() {
  ...
  async function handleCreate() {
    ...
    navigateToGame(router, `/battle/${res.code}`);
  }
  ...
  return (
    <div className="relative flex w-full min-h-screen flex-col items-center px-4 sm:px-6 py-10">
      ...
      <div className="relative z-10 w-full max-w-3xl space-y-6">
        ...
        <div className="text-center">
          ...
          <Link href="/instructions" className="mt-3 inline-block text-xs font-semibold text-emerald-400/80 transition-colors hover:text-emerald-400 hover:underline">
            📋 New here? Read how XP, Elo & battles work
          </Link>
        </div>
        ...
        <div className="grid gap-5 lg:grid-cols-2">
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-6">
            ...
            <button
              onClick={() => soloProblem && navigateToGame(router, `/solo/${soloProblem}`)}
              className="mt-4 w-full py-3 text-sm font-bold rounded-xl border border-neutral-700 text-neutral-200 transition-all duration-200 hover:border-emerald-500/30 hover:text-emerald-400 hover:bg-emerald-500/5"
            >
              Start Solo
            </button>
          </div>
          ...
          <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-6">
            <div className="mb-4 flex items-center gap-2.5">
              <span className="rounded-md bg-sky-500/10 text-sky-400 border border-sky-500/20 px-2.5 py-1 text-xs font-bold">JOIN</span>
              <h2 className="text-base font-bold text-neutral-200">Join a Room</h2>
            </div>
            <p className="text-sm text-neutral-500">Got a code from a friend? Drop it in.</p>
            <input
              value={code}
              onChange={(e) => { setCode(e.target.value.toUpperCase()); setError(null); }}
              placeholder="ROOM CODE"
              maxLength={6}
              className="mt-4 w-full rounded-lg border border-neutral-700 bg-black px-4 py-3 text-center text-lg tracking-[0.3em] font-mono text-neutral-100 outline-none transition-all duration-200 focus:border-emerald-500/40 focus:shadow-[0_0_15px_rgba(34,197,94,0.1)]"
            />
            <button
              onClick={handleJoin}
              disabled={busy}
              className="mt-4 w-full py-3 text-sm font-bold rounded-xl border border-neutral-700 text-neutral-200 transition-all duration-200 hover:border-emerald-500/30 hover:text-emerald-400 hover:bg-emerald-500/5 disabled:opacity-40"
            >
        ...
        <div className="flex items-center gap-3">
          ...
          <span className="text-xs font-bold uppercase tracking-widest text-neutral-600">or host a battle</span>
          <div className="h-px flex-1 bg-gradient-to-r from-transparent via-emerald-500/20 to-transparent" />
        </div>
        ...
        <div className="rounded-xl border border-neutral-800 bg-neutral-900/60 p-6">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <span className="rounded-md border border-neutral-700 px-2.5 py-1 text-xs font-bold text-neutral-400">
              {battleProblems.length} picked
            </span>
          </div>
          ...
          {/* Problem list */}
          <div className="mt-4 max-h-56 space-y-1 overflow-y-auto rounded-lg border border-neutral-800 bg-black p-3">
            {filtered.map((p) => {
              const picked = battleProblems.includes(p.id);
              return (
                <label
                  key={p.id}
                  className="flex cursor-pointer items-center gap-3 rounded-lg px-3 py-2 text-sm text-neutral-200 transition-colors hover:bg-emerald-500/5"
                >
                  <input
                    type="checkbox"
                    checked={picked}
            ...
            {filtered.length === 0 && (
              <p className="px-3 py-2 text-sm text-neutral-600">No problems match your filters.</p>
            )}
          </div>
          ...
          {selected.length > 0 && (
            <div className="mt-3 flex flex-wrap gap-1.5">
              {selected.map((p) => (
                <button
                  key={p.id}
                  onClick={() => toggleBattle(p.id)}
                  className="flex items-center gap-1.5 rounded-md border border-emerald-500/30 bg-emerald-500/10 px-2.5 py-1 text-xs font-medium text-emerald-300 transition-all duration-200 hover:border-[#ef4444]/40 hover:bg-[#ef4444]/10 hover:text-[#ef4444]"
                  title="Remove"
                >
                  {p.title}
